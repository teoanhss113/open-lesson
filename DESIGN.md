...
## Curriculum Workspace Application

> This section maps the Mintlify design system tokens to the AI Curriculum Workspace UI. Read this alongside the Open Design source (`open-design-main/apps/web/src/index.css`) for actual CSS variable names.

### Source of Truth

The definitive token implementation lives in:
- `open-design-main/apps/web/src/index.css` — all CSS custom properties (`--color-*`, `--text-*`, `--radius-*`, etc.)
- `open-design-main/apps/web/src/styles/` — component-scoped style files
- `open-design-main/design-systems/` — design system DESIGN.md files consumed by the daemon

Always reference `index.css` for the actual CSS variable names. The token names in the frontmatter above (`{colors.brand-green}`, etc.) are the design-spec names; the CSS variables may differ slightly in naming convention.

---

### Workspace Layout Application

The 3-panel curriculum workspace uses Open Design's `ProjectView.tsx` layout directly:

```
| DesignFilesPanel (left, ~20%)  |  FileWorkspace / FileViewer (center, ~60%)  |  ChatPane (right, ~20%)  |
```

Panel sizing follows Open Design's existing CSS grid/flex layout in `ProjectView.tsx`. Do not override panel widths with custom values — adjust only the minimum necessary for curriculum content.

#### Left Sidebar (Curriculum Explorer)

Style with:
- Nav items: `{components.sidebar-nav-item}` (transparent bg, `{colors.steel}` text, `{rounded.sm}`)
- Active nav item: `{components.sidebar-nav-item-active}` (`{colors.surface}` bg, `{colors.ink}` text)
- Section headers: `{components.sidebar-section-header}` (`{typography.micro-uppercase}`, `{colors.steel}`)
- File cards: `{components.card-base}` with `{rounded.lg}`, `{spacing.xl}` padding

Curriculum sections in sidebar (within `DesignFilesPanel` adapted component):
- All Files
- Lesson Plans
- Teaching Guides
- Slides
...
#### Main Canvas (Curriculum Artifact Workspace)

The canvas renders curriculum files via `FileViewer.tsx`:
- PDF/DOCX viewer: iframe with `{rounded.lg}` container, `{colors.hairline}` border
- HTML PPT slide decks: srcDoc render with design template from `design-templates/html-ppt-*/`
- Live Artifacts (Lesson Plan, Teaching Guide, Review): srcDoc render from `design-templates/live-artifact/`
- Code/markdown: `{components.code-block}` styling (`{colors.surface-code}` bg, `{typography.code-md}`)

Canvas text (document body):
- Use `{typography.body-md}` (Inter, 16px, weight 400, 1.50 line-height)
- Never reduce line-height below 1.50 for curriculum document prose
- Lesson headings: `{typography.heading-3}` or `{typography.heading-4}`
- Section labels: `{typography.body-sm-medium}`

#### Right Panel (AI Assistant)

Follows `ChatPane.tsx` existing styles:
- Background: `{colors.canvas}` or `{colors.surface-soft}` — flat, no shadow
- Composer input: `{components.text-input}` with `{components.text-input-focused}` on activation (mint border)
- AI messages: `{components.card-base}` treatment for assistant blocks
- Context chips (selected text): `{components.badge-tag}` style (`{colors.brand-tag}` tint)
- Send button: `{components.button-primary}` pill
...
Use `{components.badge-required}` pattern for curriculum risk levels:
- HIGH risk: `{colors.brand-error}` background, `{colors.on-dark}` text, `{typography.micro-uppercase}`, `{rounded.sm}`
- MEDIUM risk: `{colors.brand-warn}` background, `{colors.primary}` text (same badge shape)
- LOW risk: `{colors.surface}` background, `{colors.steel}` text
- PASSED: `{colors.brand-green}` background, `{colors.primary}` text

Do NOT create custom badge components — extend the existing badge pattern from `index.css`.

#### Curriculum Status Tags

Use `{components.badge-type}` pattern:
- `{colors.surface}` bg, `{colors.steel}` text, `{typography.code-sm}`, `{rounded.sm}`
- Labels: `draft` · `in-review` · `approved` · `archived`

#### Artifact Tabs

Reuse `WorkspaceTabsBar.tsx` for curriculum artifact tabs:
- Lesson Plan tab — `live:` prefix tab (LiveArtifact)
- Teaching Guide tab — `live:` prefix tab
- Slides tab — file tab or `live:` tab
- Analysis tab — `live:` prefix tab
- Inactive tab: `{components.segmented-tab}` style
- Active tab: `{components.segmented-tab-active}` (2px bottom border in `{colors.ink}`)

#### Buttons

- Primary AI action: `{components.button-primary}` (black pill)
- Generate Lesson Plan: `{components.button-accent-green}` (mint pill, reserved for primary generation CTA)
- Secondary actions: `{components.button-secondary}` (outlined pill)
- Sidebar icon actions: `{components.button-ghost}` (transparent, `{rounded.md}`)
- Icon-only actions: `{components.button-icon-circular}` (32×32px, `{colors.canvas}` bg, `{colors.hairline}` border)

#### Cards

- Curriculum file cards (in `DesignFilesPanel`): `{components.card-base}` — canvas bg, `{rounded.lg}`, `{spacing.xl}` padding, hairline border
- AI analysis result panels: `{components.card-feature}` — surface bg, `{rounded.lg}`, `{spacing.xxl}` padding
- Risk report cards: `{components.card-base}` with risk-level badge top-right
...
| Surface | Token |
|---|---|
| Curriculum library hero | `{typography.heading-2}` or `{typography.heading-1}` |
| Lesson title in canvas | `{typography.heading-3}` (28px) |
| Section headers (LP, TG) | `{typography.heading-4}` (22px) |
| Sidebar section labels | `{typography.micro-uppercase}` (11px, uppercase, +0.5px letter-spacing) |
| Sidebar file names | `{typography.body-sm}` (14px) |
| Sidebar active file name | `{typography.body-sm-medium}` (14px, weight 500) |
| Curriculum document body | `{typography.body-md}` (16px, 1.50 line-height) — never compress |
| AI message body | `{typography.body-md}` (16px) |
| AI assistant caption | `{typography.caption}` (13px) |
| Risk level badge text | `{typography.micro-uppercase}` |
| Status badge text | `{typography.caption-bold}` |
| Code in feedback/notes | `{typography.code-md}` via Geist Mono |

---

### Animation

Follow Open Design's animation philosophy (`AGENTS.md` → UI animation philosophy):
- Default ease-out: `cubic-bezier(0.23, 1, 0.32, 1)`
- Enter: ~200ms, Exit: ~140ms
- Accordion expand/collapse: `grid-template-rows: 0fr → 1fr` + opacity fade
- Never scale from 0 — start from `scale(0.9)` + `opacity: 0`
- Never `ease-in` for UI elements
- Use `.accordion-collapsible` + `.accordion-collapsible-inner` pattern from `index.css`

Curriculum-specific animations:
- Risk badge appear: `scale(0.9) → scale(1)` + opacity, 200ms ease-out
- Lesson Plan generation streaming: no animation on content — only the loading spinner
- Panel slide-in: `translateX(8px) → translateX(0)` + opacity, 200ms ease-out

---

### Color Usage in Curriculum Workspace

| Element | Color Token | Notes |
|---|---|---|
| Primary CTA (Generate) | `{colors.brand-green}` | Reserved — only on primary generation button |
| Pill buttons (standard) | `{colors.primary}` (black) | Black pill, white text |
| Sidebar inactive text | `{colors.steel}` | All sidebar nav items by default |
| Sidebar active text | `{colors.ink}` | Active curriculum section |
| Canvas background | `{colors.canvas}` | Pure white — keeps content primary |
| AI panel background | `{colors.surface-soft}` or `{colors.canvas}` | Subtle separation from canvas |
| Section dividers | `{colors.hairline}` | 1px borders |
| HIGH risk badge | `{colors.brand-error}` | Reserved for rollout risk only |
| MEDIUM risk badge | `{colors.brand-warn}` | Warning level |
| PASSED status | `{colors.brand-green}` | Confirmation — same mint as brand-green |
| Code / feedback | `{colors.surface-code}` | Dark code surface for extracted text |

### What Not to Do in Curriculum UI

- Do NOT use `{colors.brand-green}` for generic status chips — reserve it for generation CTAs and PASSED status
- Do NOT add hero gradient bands (sky-blue, dark-teal) inside the 3-panel workspace — those are for marketing pages only
- Do NOT reduce `{typography.body-md}` line-height in curriculum document viewer — readability is required
- Do NOT introduce a third typeface — Inter (UI) + Geist Mono (code) only
- Do NOT use rectangular (non-pill) buttons for user-facing actions
- Do NOT add modal popups that overlap the canvas — use the right panel (`ChatPane`) for AI interactions
- Do NOT build a separate chatbot dialog — all AI interaction goes through the existing `ChatPane` / `ChatComposer`
