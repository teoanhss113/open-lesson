...
export interface CurriculumDefaultsPrefs {
...
export interface AppConfigPrefs {
  ...
  curriculumDefaults?: CurriculumDefaultsPrefs;
}
...
function validateCurriculumDefaults(raw: unknown): CurriculumDefaultsPrefs | undefined {
  if (raw === undefined || raw === null) return undefined;
  if (typeof raw !== 'object' || Array.isArray(raw)) return undefined;
  const obj = raw as Record<string, unknown>;
  const defaults: CurriculumDefaultsPrefs = {};
  if (typeof obj.defaultAgeGroup === 'string') defaults.defaultAgeGroup = obj.defaultAgeGroup;
  if (typeof obj.defaultLevel === 'string') defaults.defaultLevel = obj.defaultLevel;
  if (typeof obj.defaultSkillId === 'string') defaults.defaultSkillId = obj.defaultSkillId;
  return defaults;
}
...
function applyConfigValue(
  ...
  if (key === 'curriculumDefaults') {
    const validated = validateCurriculumDefaults(value);
    if (validated !== undefined) {
      target[key] = validated;
    } else {
      delete target[key];
