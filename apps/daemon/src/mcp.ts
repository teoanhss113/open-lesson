...
// (Claude Code, Cursor, Zed) pull files from a local Curriculum Workspace
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no activity).',
} as const;

const TOOL_DEFS = [
  {
    name: 'list_projects',
    description: 'List every project on this daemon.',
  ...
  },
  {
    ...
    description:
      'Project + file the user has open in the workspace right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with the workspace (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no activity.",
        },
        include: {
          type: 'string',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no activity.',
        },
        offset: {
          type: 'number',
...
export async function runMcpStdio({ daemonUrl }: RunMcpOptions): Promise<void> {
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        'Curriculum Workspace (CW) is a local-first curriculum management workspace. The user typically',
        ...
        '',
        'When extending a design in another codebase, pull',
    ...
    const resources = [
      {
        ...
        description: 'The project/file the user has open in the workspace right now.',
      ...
      switch (name) {
        ...
        case 'get_active_context': {
          ...
          if (!data || data.active === false) {
            return ok({
              active: false,
              hint: 'The workspace has no active project right now. The active context expires about 5 minutes after the last user interaction with the workspace, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
            });
          }
          return ok(data);
        }
...
async function resolveProjectArg(baseUrl: string, arg: unknown): Promise<{ id: string; resolved: ResolvedProject | null; active: ActiveContext | null }> {
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and The workspace has no active project. The active context expires about 5 minutes after the last user interaction with the workspace - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the Curriculum Workspace daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
