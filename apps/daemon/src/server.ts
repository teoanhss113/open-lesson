...
export async function startServer({
  ...
  app.get('/api/projects/:id/files/:name/preview', async (req, res) => {
    try {
      ...
      const preview = await buildDocumentPreview(file, projectDir);
      res.json(preview);
    } catch (err) {
      const status =
        err && err.statusCode
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    if (cwd && safeAttachments.length > 0) {
      ...
      for (const attachmentRelPath of safeAttachments) {
        try {
          const absPath = path.resolve(cwd, attachmentRelPath);
          const name = path.basename(absPath);
          const ext = path.extname(name).toLowerCase();
          
          if (['.pdf', '.docx', '.pptx', '.xlsx'].includes(ext)) {
            const buffer = await fs.promises.readFile(absPath);
            const preview = await buildDocumentPreview({ name, buffer }, cwd);
          ...
          console.warn(`[chat-attachments] Failed to preprocess attachment "${attachmentRelPath}":`, err);
        }
      }
      
      if (parsedAttachments.length > 0) {
        attachmentsPreviewBlock = '\n\n## Attached Documents Content & Asset References\n' +
          parsedAttachments.map((pa) => {
            const sectionsStr = pa.sections.map((sec) => {
