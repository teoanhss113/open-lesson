...
function renderMetadataBlock(
  ...
  if (metadata.intent === 'live-artifact') {
    ...
    lines.push(
      '- **A4-Style Paginated Documents Rule**: When generating structured curriculum documents, syllabus, lesson plans, teaching guides, or educational reports, you must present the content as a realistic sequence of A4 pages. Follow this visual specification exactly:\n' +
      '  - **Structural Blueprint**: Wrap the entire content within a `<div class="document-container">`. Each individual page must be styled as a separate container `<div class="a4-page portrait">` (or `<div class="a4-page landscape">` for wide/horizontal tables). Every page must contain a `.page-header` block at the top, a `.page-content` block in the middle, and a `.page-footer` block at the bottom.\n' +
      '  - **Typography & Styling**: Apply modern typography (e.g., using imported Google Fonts like Inter or Outfit). Use the brand-green, dark primary, and elegant steel accent colors in HSL or hexadecimal. Standard A4 pages should be styled with a clean paper sheet look (e.g., shadow `box-shadow: 0 10px 25px -5px rgba(0,0,0,0.05), 0 8px 10px -6px rgba(0,0,0,0.05)`, white background, light gray borders) on a soft gray body background (#f8fafc slate or equivalent) to wow the user. \n' +
      '  - **CSS Blueprint to embed in <style>**:\n' +
      '    ```css\n' +
      '    body {\n' +
      ...
      '       - **Documents and Presentations (.docx, .pptx, .html, or previous Live Artifacts)**: Must be treated strictly as **Layout, Design, and Styling Templates**.\n' +
      '    2. **Execute Design & Layout Cloning**:\n' +
      '       - Replicate the exact design system, styling, color palette (hex/rgb), typography, borders, table structures, and page settings of the template document.\n' +
      '       - Keep the page headers, footers, logo placement, page margin/padding, and branding elements completely identical to the template document.\n' +
      '       - Apply the new content (e.g., Lesson 2 / Buổi 2) parsed from the spreadsheet source file into this exact cloned layout structure.\n' +
      '    3. **Enhance with Images from Internet**:\n' +
      '       - Actively locate relevant image sources online (or generate valid web URLs for images related to the lesson topic, e.g. Robots, AI, STEAM) and embed them directly using `<img src="..." alt="...">` with descriptive captions. Do not use generic text placeholders for images.',
    );
  }
