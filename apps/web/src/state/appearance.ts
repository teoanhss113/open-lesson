...
export const DEFAULT_ACCENT_COLOR = '#00d4a4';
...
function accentVars(accentColor: string): Record<(typeof ACCENT_VARS)[number], string> {
  return {
    ...
    '--accent-soft': `color-mix(in srgb, ${accentColor} 48%, var(--bg-panel))`,
    '--accent-tint': `color-mix(in srgb, ${accentColor} 8%, var(--bg-panel))`,
