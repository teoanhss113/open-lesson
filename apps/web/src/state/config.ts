...
export const DEFAULT_CURRICULUM_DEFAULTS: CurriculumDefaultsConfig = {
  defaultAgeGroup: 'K-12',
  defaultLevel: 'Beginner',
  defaultSkillId: 'curriculum-analysis',
};
...
export const DEFAULT_CONFIG: AppConfig = {
  ...
  orbit: DEFAULT_ORBIT,
  curriculumDefaults: DEFAULT_CURRICULUM_DEFAULTS,
};
...
function normalizeCurriculumDefaults(
  input: Partial<CurriculumDefaultsConfig> | undefined,
): CurriculumDefaultsConfig {
  if (!input) return { ...DEFAULT_CURRICULUM_DEFAULTS };
  return {
    ...DEFAULT_CURRICULUM_DEFAULTS,
    ...input,
  };
}
  ...
  try {
    ...
    if (!raw) {
      return {
        ...
        curriculumDefaults: normalizeCurriculumDefaults(DEFAULT_CURRICULUM_DEFAULTS),
      };
    }
    ...
    const merged: AppConfig = {
      ...
      accentColor: (() => {
        const val = normalizeAccentColor(parsed.accentColor);
        return (!val || val === '#c96442') ? DEFAULT_CONFIG.accentColor : val;
      })(),
      pet: normalizePet(parsed.pet),
      notifications: normalizeNotifications(parsed.notifications),
      orbit: normalizeOrbit(parsed.orbit),
      curriculumDefaults: normalizeCurriculumDefaults(parsed.curriculumDefaults),
    };
...
export function mergeDaemonConfig(
  ...
  if (daemonConfig.curriculumDefaults !== undefined) {
    next.curriculumDefaults = normalizeCurriculumDefaults(daemonConfig.curriculumDefaults);
  }
  return next;
}
...
export async function syncConfigToDaemon(
  ...
  const prefs: AppConfigPrefs = {
    ...
    curriculumDefaults: config.curriculumDefaults ? {
      defaultAgeGroup: config.curriculumDefaults.defaultAgeGroup,
      defaultLevel: config.curriculumDefaults.defaultLevel,
      defaultSkillId: config.curriculumDefaults.defaultSkillId,
    } : undefined,
  };
  try {
