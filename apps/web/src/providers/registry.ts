...
export interface UpdateCurriculumStatusResult {
  project?: Project;
  error?: string;
  message?: string;
  blockers?: CurriculumRisk[];
}

export async function updateCurriculumStatus(
  projectId: string,
  curriculumStatus: string,
): Promise<UpdateCurriculumStatusResult> {
  try {
    const resp = await fetch(`/api/curriculum/projects/${encodeURIComponent(projectId)}/status`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ curriculumStatus }),
    });
    const json = await resp.json();
    if (!resp.ok) {
      return {
        error: json.error ?? 'ROLLOUT_VALIDATION_FAILED',
        message: json.message ?? 'Validation failed',
        blockers: json.blockers ?? [],
      };
    }
    return json as UpdateCurriculumStatusResult;
  } catch (err) {
    return {
      error: 'NETWORK_ERROR',
      message: err instanceof Error ? err.message : 'Network error',
