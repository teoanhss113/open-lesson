...
function injectSelectionTracker(doc: string): string {
  const script = `<script data-od-selection-tracker>(function(){
  document.addEventListener('mouseup', function() {
    setTimeout(function() {
      var sel = window.getSelection();
      var txt = sel ? sel.toString().trim() : '';
      window.parent.postMessage({ type: 'od:selection', text: txt }, '*');
    }, 10);
  });
})();</script>`;
...
.deck-stage { position: absolute !important; top: 0 !important; left: 0 !important; }
</style>`;
