...
export function WorkspaceTabsBar({ route, projects }: Props) {
  ...
  return (
    <header className="app-chrome-header workspace-tabs-chrome" aria-label={t('workspaceTabs.label')}>
      ...
      >
        ...
        {hiddenTabCount > 0 ? (
          <button
            ...
            title={t('workspaceTabs.showHidden')}
          >
...
function displayTabFor(
  ...
  if (tab.kind === 'project') {
    ...
    return {
      ...
      meta: t('workspaceTabs.projectMeta'),
      icon: 'folder',
      tab,
    };
  }
  if (tab.kind === 'marketplace') {
    return {
      ...
      meta: t('workspaceTabs.pluginsMeta'),
      icon: 'grid',
      tab,
    };
  }
