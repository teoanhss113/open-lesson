...
export const HomeHero = forwardRef<HTMLTextAreaElement, Props>(function HomeHero(
  ...
  return (
    <section className="home-hero" data-testid="home-hero">
      ...
      <h1 className="home-hero__title">{t('curriculum.hero.title')}</h1>
        ...
        <div className="home-hero__input-foot">
          ...
          <div className="home-hero__foot-left">
            ...
            <span className="home-hero__hint">
              <kbd>↵</kbd> {t('homeHero.keyboardHint')}
            </span>
          </div>
          <button
            type="button"
            ...
            onClick={onSubmit}
            disabled={!canSubmit}
            title={canSubmit ? t('homeHero.run') : t('homeHero.runDisabledTitle')}
  ...
  return (
    ...
    >
      {chips.map((chip) => {
        ...
        const hint = homeHeroChipHint(chip, t) ?? label;
        return (
          <button
            key={chip.id}
            type="button"
...
function homeHeroChipLabel(chip: HomeHeroChip, t: ReturnType<typeof useI18n>['t']): string {
  const keys: Record<string, Parameters<typeof t>[0]> = {
    prototype: 'homeHero.chip.prototype',
    'live-artifact': 'homeHero.chip.liveArtifact',
    deck: 'homeHero.chip.deck',
    image: 'homeHero.chip.image',
    video: 'homeHero.chip.video',
    hyperframes: 'homeHero.chip.hyperframes',
    audio: 'homeHero.chip.audio',
    'create-plugin': 'homeHero.chip.createPlugin',
    figma: 'homeHero.chip.fromFigma',
    folder: 'homeHero.chip.fromFolder',
    template: 'homeHero.chip.fromTemplate',
  };
  const key = keys[chip.id];
  return key ? t(key) : chip.label;
}

function homeHeroChipHint(chip: HomeHeroChip, t: ReturnType<typeof useI18n>['t']): string | null {
  const keys: Record<string, Parameters<typeof t>[0]> = {
    'live-artifact': 'homeHero.hint.liveArtifact',
    hyperframes: 'homeHero.hint.hyperframes',
    'create-plugin': 'homeHero.hint.createPlugin',
    figma: 'homeHero.hint.fromFigma',
    folder: 'homeHero.hint.fromFolder',
    template: 'homeHero.hint.fromTemplate',
  };
  const key = keys[chip.id];
  return key ? t(key) : chip.hint ?? null;
}
