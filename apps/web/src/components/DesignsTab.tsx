...
export function DesignsTab({
	...
	const handleStatusUpdate = async (projectId: string, nextStatus: 'draft' | 'in-review' | 'approved' | 'archived') => {
		setValidatingProjectId(projectId);
		setDesignsTabBlockers((curr) => {
			const next = { ...curr };
		...
		try {
			...
			if (result.error === 'ROLLOUT_VALIDATION_FAILED' && result.blockers) {
				setDesignsTabBlockers((curr) => ({
					...curr,
					[projectId]: result.blockers ?? [],
				}));
			} else if (result.error) {
				console.error('[handleStatusUpdate] Error changing status:', result.message);
			} else {
			...
			console.error('[handleStatusUpdate] Error updating curriculum status:', err);
		} finally {
										...
										>
											...
											{p.metadata?.curriculumKind && (
												<>
													...
													{p.metadata.curriculumStatus !== 'in-review' && (
														<button
															type="button"
															role="menuitem"
															onClick={() => {
													...
													{p.metadata.curriculumStatus !== 'approved' && (
														<button
															type="button"
															role="menuitem"
															onClick={() => {
													...
													{p.metadata.curriculumStatus !== 'archived' && (
														<button
															type="button"
															role="menuitem"
															onClick={() => {
								...
								<div className="design-card-meta-block">
									...
									{p.metadata?.curriculumKind ? (
										<div className="design-card-badges" style={{ marginTop: 'var(--spacing-xxs)', marginBottom: 'var(--spacing-xxs)' }}>
											...
											{p.metadata.overallRisk && (
												<CurriculumRiskBadge risk={p.metadata.overallRisk} />
											)}
										</div>
									) : null}
										...
										colProjects.map(({ project: p }) => {
											...
											return (
												...
												>
													...
													{p.metadata?.curriculumKind && (
														<div
															className="design-card-menu-anchor design-card-menu-anchor--kanban"
															ref={menuOpenId === p.id ? menuContainerRef : undefined}
														>
															<button
																type="button"
